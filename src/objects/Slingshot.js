...
import Input from "../../lib/Input.js"; // For detecting spacebar input
import {
  canvas,
  CANVAS_HEIGHT,
  CANVAS_WIDTH,
...
const { Body, Composite, Constraint, Events, Mouse, MouseConstraint, Vector } =
  matter;
...
export default class Slingshot {
  ...
  update(dt) {
    ...
    // Check for spacebar input to activate bird's special ability.
    // Only activates if bird exists, is in flight, and has a useAbility method.
    if (input.isKeyPressed(Input.KEYS.SPACE)) {
      if (
        this.bird &&
        !this.bird.isWaiting &&
        typeof this.bird.useAbility === "function"
      ) {
  ...
  renderSlingBack() {
    ...
    context.lineTo(
      this.sling.bodyB?.position.x - this.bird.radius * 0.7,
      this.sling.bodyB?.position.y + this.bird.radius * 0.7
    );
    context.closePath();
    ...
    this.sprites[Slingshot.BACK_SPRITE.index].render(
      this.sling.pointA.x,
      this.sling.pointA.y - 50
    );
  }
  ...
  renderSlingFront() {
    ...
    this.sprites[Slingshot.FRONT_SPRITE.index].render(
      this.sling.pointA.x - 28,
      this.sling.pointA.y - 55
    );
  }
  ...
  static generateSprites() {
    return [
      new Sprite(
        ...
        Slingshot.BACK_SPRITE.x,
        Slingshot.BACK_SPRITE.y,
        Slingshot.BACK_SPRITE.width,
        Slingshot.BACK_SPRITE.height
      ),
      new Sprite(
        images.get(ImageName.Sprites),
        Slingshot.FRONT_SPRITE.x,
        Slingshot.FRONT_SPRITE.y,
        Slingshot.FRONT_SPRITE.width,
        Slingshot.FRONT_SPRITE.height
      ),
    ];
  }
  ...
  isReadyToLaunch() {
    return (
      this.mouseWasReleased &&
      ((this.mouse.position.x < this.sling.pointA.x &&
        this.sling.bodyB?.position.x > this.sling.pointA.x + 3) ||
        (this.mouse.position.x > this.sling.pointA.x &&
          this.sling.bodyB?.position.x < this.sling.pointA.x - 3))
    );
  }
  ...
  shouldUnload() {
    return (
      this.wasLaunched && (this.bird.didStop() || this.bird.didGoOffScreen())
    );
  }
